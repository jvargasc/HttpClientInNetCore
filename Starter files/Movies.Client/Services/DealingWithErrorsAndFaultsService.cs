...
namespace Movies.Client.Services
{
    public class DealingWithErrorsAndFaultsService : IIntegrationService
    {
        ...
        {
			//await GetMovieAndDealWithInvalidResponses(_cancellationToken);
			await PostMovieAndHandleValidationErrors(_cancellationToken);
		}
				...
				cancellationToken))
			{
				if (!response.IsSuccessStatusCode)
				{
					...
					else if (response.StatusCode == System.Net.HttpStatusCode.Unauthorized)
					{
						...
						Console.WriteLine("trigger a login flow");
						return;
					}
